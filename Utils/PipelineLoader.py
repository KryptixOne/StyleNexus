from diffusers import StableDiffusionInpaintPipeline, StableDiffusionImg2ImgPipeline,StableDiffusionPipeline, DDIMScheduler
import torch

def build_SD_pipeline_based_on_input(checkpoint_path: str, device: str = 'cuda', pipeline_type: str = None, **kwargs):
    """
    ...
    assert pipeline_type in valid_pipeline_types, 'pipeline specified cannot be NoneType. Choose from the following: {}'.format(valid_pipeline_types)

    assert checkpoint_path, 'Checkpoint_path cannot be NoneType'

    if pipeline_type == 'Inpaint':
        pipe = StableDiffusionInpaintPipeline.from_pretrained(checkpoint_path, torch_dtype=torch.float16)
