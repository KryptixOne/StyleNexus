...
text_input = tokenize(
    prompt,
    padding="max_length",
    max_length=tokenize.model_max_length,
    truncation=True,
    return_tensors="pt",
)
...
latents = torch.randn(
    (batch_size, unet.config.in_channels, height // 8, width // 8), generator=generator
)  # generator is the seed
...
for t in tqdm(scheduler.timesteps):
    latent_model_input = torch.cat(
        [latents] * 2
    )  # expand latents if we are doing CFG to avoid doing two forward passes

    latent_model_input = scheduler.scale_model_input(latent_model_input, timestep=t)

    with torch.no_grad():
        noise_pred = unet(
            latent_model_input.to(torch_device),
            t,
